...
int orb_save_descriptor(mp_obj_t fp, array_t *kpts)
{
    ...
    res = file_write(fp, &kpts_size, sizeof(kpts_size), &bytes);
    ...
    for (int i=0; i<kpts_size; i++) {
        ...
        // Write X
        res = file_write(fp, &kp->x, sizeof(kp->x), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->x)) {
            goto error;
        }

        // Write Y
        res = file_write(fp, &kp->y, sizeof(kp->y), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->y)) {
            goto error;
        }

        // Write Score
        res = file_write(fp, &kp->score, sizeof(kp->score), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->score)) {
            goto error;
        }

        // Write Octave
        res = file_write(fp, &kp->octave, sizeof(kp->octave), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->octave)) {
            goto error;
        }

        // Write Angle
        res = file_write(fp, &kp->angle, sizeof(kp->angle), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->angle)) {
            goto error;
        }

        // Write descriptor
        res = file_write(fp, kp->desc, KDESC_SIZE, &bytes);
...
int orb_load_descriptor(mp_obj_t fp, array_t *kpts)
{
    ...
    res = file_read(fp, &kpts_size, sizeof(kpts_size), &bytes);
    if (res != FR_OK || bytes != sizeof(kpts_size)) {
        goto error;
    }
    ...
    for (int i=0; i<kpts_size; i++) {
        ...
        // Read X
        res = file_read(fp, &kp->x, sizeof(kp->x), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->x)) {
            goto error;
        }

        // Read Y
        res = file_read(fp, &kp->y, sizeof(kp->y), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->y)) {
            goto error;
        }

        // Read Score
        res = file_read(fp, &kp->score, sizeof(kp->score), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->score)) {
            goto error;
        }

        // Read Octave
        res = file_read(fp, &kp->octave, sizeof(kp->octave), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->octave)) {
            goto error;
        }

        // Read Angle
        res = file_read(fp, &kp->angle, sizeof(kp->angle), &bytes);
        if (res != FR_OK || bytes != sizeof(kp->angle)) {
            goto error;
        }

        // Read descriptor
        res = file_read(fp, kp->desc,  KDESC_SIZE, &bytes);
        if (res != FR_OK || bytes != KDESC_SIZE) {
            goto error;
        }
