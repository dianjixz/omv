...
#if 1
# define ASSERT_POS \
    configASSERT(p == data + x + y * (intptr_t)w)
#else
# define ASSERT_POS
#endif
                               ...
                               int datalen)
{
    ...
    if(sym) {
        ...
        sym->next = NULL;
        configASSERT(iscn->recycle[i].nsyms);
...
static void symbol_handler (zbar_decoder_t *dcode)
{
    ...
    /* FIXME configASSERT(type == ZBAR_PARTIAL) */
